...
# Free historical DRAM spot data is not reliably available for Streamlit/backtests.
# So the model uses Micron (MU) and SOXX as a daily liquid proxy for the DRAM / memory cycle.
...
def memory_cycle_momentum(market_prices):
    """
    Liquid proxy for DRAM / memory-cycle strength.

    Direct historical DRAM spot prices are not reliably available as a free API.
    This proxy uses:
      - MU: Micron, a memory-cycle exposed equity
      - SOXX: broad semiconductor leadership

    Output is a 30-day return in decimal form.
    """
...
def memory_factor_contribution(sector, market_prices):
    """
    Sector-specific contribution from memory-cycle momentum.
    Kept deliberately modest so it tweaks rather than dominates the model.
    """
...
def market_macro_score(sector, ticker, prices, market_prices):
    ...
    # DRAM / memory-cycle tweak: positive when memory-sensitive equities lead.
...
st.caption(
    ...
    "AAII is not included because it is not reliably available as a free machine-readable daily feed. The DRAM/memory-cycle tweak uses MU and SOXX as liquid historical proxies rather than scraped spot DRAM prices."
)
...
st.write(
    ...
    "and positive implied earnings-revision pressure. The long book is intentionally not over-gated by broad defensive filters. A modest DRAM / memory-cycle proxy is included via MU and SOXX."
)
